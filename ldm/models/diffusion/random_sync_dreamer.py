...
def visual_camera(c2w, intrisics, idx=0, points=None):
    ...
    for i in range(0, num_frames):
        ...
        pc.plot_camera(
            ax, cam2world=cam2world, M=intrinsic_matrix, sensor_size=np.array([256, 256]),
...
def batch_gather(data, index, dim = 1):
...
class SpatialVolumeNet(nn.Module):
    ...
    def construct_spatial_volume(self, x, t_embed, v_embed, target_poses, target_Ks):
        ...
        # w2c = target_poses[0] # N,3,4
        # K = target_Ks[0] # N,3,3
        # # 3 4 - > 4 4
        # w2c = torch.cat([w2c, torch.tensor([0,0,0,1], dtype=torch.float32, device=device).view(1,1,4).repeat(N,1,1)], 1)
        # K = torch.cat([K, torch.zeros(N,1,3, dtype=torch.float32, device=device)], 1)
        # c2w = torch.inverse(w2c)
        # visual_camera(c2w[:1], K[:1], 0, spatial_volume_verts[0].reshape(3, -1).permute(1, 0))
        # target_Ks = target_Ks.unsqueeze(1).repeat(1, N, 1, 1)
    ...
    def construct_view_frustum_volume(self, spatial_volume, t_embed, v_embed, poses, Ks, target_indices):
        ...
        poses_ = batch_gather(poses, target_indices).reshape(-1, 3, 4)
        Ks_ = batch_gather(Ks, target_indices).reshape(-1, 3, 3)
        # print(poses_.shape)
        # print(Ks.shape)
...
class SyncMultiviewDiffusion(pl.LightningModule):
    def __init__(self, unet_config, scheduler_config,
                 ...
                 clip_image_encoder_path="ViT-L/14",
    ...
    def get_viewpoint_embedding(self, elevation, azimuth, distance):
        """
        @param batch_size:
    ...
    def prepare(self, batch):
        ...
        input_info = { 'image': image_input, 
                       'elevations': batch['elevations'], 
                       "azimuths": batch['azimuths'],
                       "distances": batch['distances'],
                       "poses": batch['w2c_matrixs'][:, :, :3, :4],
                       "Ks": batch['intrinsics'][:, :, :3, :3],
                       'x': x_input}
    ...
    def get_target_view_feats(self, x_input, spatial_volume, clip_embed, t_embed, v_embed, target_index, poses, Ks):
        """
        ...
        frustum_volume_feats, frustum_volume_depth = self.spatial_volume.construct_view_frustum_volume(spatial_volume, t_embed, v_embed, poses, Ks, target_index)
    ...
    def training_step(self, batch):
        ...
        v_embed = self.get_viewpoint_embedding(input_info['elevations'],
                                               input_info['azimuths'],
                                               input_info['distances']
                                               ) # N,v_dim

        t_embed = self.embed_time(time_steps)
        spatial_volume = self.spatial_volume.construct_spatial_volume(x_noisy, t_embed, v_embed, input_info['poses'], input_info['Ks'])

        clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], spatial_volume, clip_embed, t_embed, v_embed, target_index, input_info['poses'], input_info['Ks'])
...
class SyncDDIMSampler:
    ...
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale, batch_view_num=1, is_step0=False):
        ...
        v_embed = self.model.get_viewpoint_embedding(input_info['elevations'],
                                               input_info['azimuths'],
                                               input_info['distances']
                                               ) # N,v_dim
        t_embed = self.model.embed_time(time_steps)  # B,t_dim
        spatial_volume = self.model.spatial_volume.construct_spatial_volume(x_target_noisy, t_embed, v_embed, input_info['poses'], input_info['Ks'])

        e_t = []
        target_indices = torch.arange(N).to(x_input.device) # N
        for ni in range(0, N, batch_view_num):
            ...
            clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, spatial_volume, clip_embed, t_embed, v_embed, target_indices_, input_info['poses'], input_info['Ks'])
